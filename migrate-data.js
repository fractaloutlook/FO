// Data migration script for copying data from status-module to status-module-v2
// Run this script after both modules are published

// This script should be run in a Node.js environment with the SpacetimeDB SDK
...
async function migrateData() {
  ...
  try {
    ...
    const { DbConnection } = await import('./src/module_bindings/index.js');
    ...
    for (const user of users) {
      try {
        if (user.name) {
          // Note: This will set the name for the current user context
          // You may need to handle this differently depending on your setup
