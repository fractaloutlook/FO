...
const PollSystem = ({ connection, isAdmin }) => {
    ...
    if (connection) {
      ...
      if (connection.db.poll) {
        ...
        if (active) {
          ...
          const options = Array.from(connection.db.pollOption.iter())
            .filter(o => o.pollId === active.pollId);
            
          // Get vote counts
          const votes = Array.from(connection.db.pollVote.iter())
            .filter(v => v.pollId === active.pollId);
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-4 my-4">
      <h3 className="font-medium text-gray-900 mb-2">Community Poll</h3>
                ...
                <div key={option.optionId} className="relative">
                  <div 
                    className={`p-3 rounded ${
                      selectedOption === option.optionId ? 'bg-blue-100 border border-blue-300' : 'bg-gray-50'
                    }`}
                    onClick={() => setSelectedOption(option.optionId)}
                  >
                    <div className="flex justify-between">
                      <span>{option.text}</span>
                      <span className="text-gray-600">{count} votes ({percent}%)</span>
                    </div>
